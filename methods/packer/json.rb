...
def create_packer_json(values)
  ...
  disk_adapter_type = 'nvme' if values['vm'].to_s.match(/fusion/) && (hw_version.to_i >= 20)
  install_password = if values['answers']['admin_password']
                       values['answers']['admin_password'].value
                     else
                       values['answers']['root_password'].value
                     end
  if values['vm'].to_s.match(/parallels/)
    parallels_tools_flavor = case values['service'].to_s
                             when /win/
                               'win'
  ...
  http_dir = "#{packer_dir}/#{values['vm']}/#{values['name']}/subiquity/http" if values['livecd'] == true
  ethernet_type = if values['dhcp'] == true
  ...
  headless_mode = values['answers']['headless_mode'].value unless values['service'].to_s.match(/purity/)
  if values['method'].to_s.match(/vs/)
    admin_crypt = values['answers']['root_crypt'].value
  else
    admin_crypt = values['answers']['admin_crypt'].value unless values['service'].to_s.match(/win|sol_[9,10]/)
  end
  guest_additions_mode = if values['answers']['guest_additions_mode']
                           values['answers']['guest_additions_mode'].value
                         else
                           values['vmtools']
                         end
  if values['vmnetwork'].to_s.match(/hostonly/)
    ks_ip = if values['httpbindaddress'] != values['empty']
              if values['vn'].to_s.match(/fusion/)
                values['vmgateway']
              else
                values['httpbindaddress']
              end
    ...
    "packerssh,tcp,#{values['ip']},#{ssh_port},#{values['ip']},22"
  else
    ks_ip = if values['httpbindaddress'] != values['empty']
              values['httpbindaddress']
            elsif values['vm'].to_s.match(/fusion/) && (values['dhcp'] == true)
              values['vmgateway']
            else
              values['hostip']
            end
  ...
  if values['ip'].to_s.match(/[0-9]/)
    ...
    port_no = "0#{port_no}" if port_no.to_i < 100
  ...
  tools_upload_flavor = ''
  tools_upload_path   = ''
  if values['vmnetwork'].to_s.match(/hostonly/) && values['vm'].to_s.match(/vbox/)
    get_bridged_vbox_nic(values)
    nic_name = check_vbox_hostonly_network(values)
    nic_command1 = '--nic1'
    nic_config1  = 'hostonly'
    nic_command2 = '--nictype1'
    nic_config2 = if values['service'].to_s.match(/vmware|esx|vsphere/)
                    'virtio'
                  else
                    '82545EM'
                  end
    nic_command3 = '--hostonlyadapter1'
    nic_config3  = nic_name.to_s
    ks_ip = if values['httpbindaddress'] != values['empty']
              values['httpbindaddress']
            else
              values['vmgateway']
  ...
  if values['vmnetwork'].to_s.match(/bridged/) && values['vm'].to_s.match(/vbox/)
    nic_name = get_bridged_vbox_nic(values)
    nic_command1 = '--nic1'
    nic_config1  = 'bridged'
    nic_command2 = '--nictype1'
    nic_config2 = if values['service'].to_s.match(/vmware|esx|vsphere/)
  ...
  virtual_dev = 'pvscsi' if values['service'].to_s.match(/[el,centos,rocky,alma]_8/)
  ...
  values['answers']['root_password'].value unless values['service'].to_s.match(/win|purity/)
  shutdown_command = ''
  mac_address = generate_mac_address(values['vm']) unless mac_address.to_s.match(/[0-9]/)
  values['guest'] = values['guest'].join if values['guest'].instance_of?(Array)
  if values['service'].to_s.match(/sol/)
    if values['copykeys'] == true && File.exist?(values['sshkeyfile'].to_s)
      ssh_key = `cat #{values['sshkeyfile']}`.chomp
  ...
  when /win/
    if (values['vmtools'] == true) && !values['label'].to_s.match(/2016|2019/)
    ...
    if values['label'].to_s.match(/20[0,1][0-8]/)
      ...
      memsize = '2048' if memsize.to_i < 2000
    elsif values['vm'].to_s.match(/fusion/)
  ...
  when /sol_11_[2-3]/
    if (values['host-os-cpu'].to_i > 6) && (values['host-os-memory'].to_i > 16)
      wait_time1 = '<wait120>'
      wait_time2 = '<wait90>'
    else
      wait_time1 = '<wait160>'
    ...
    memsize = '2048' if memsize.to_i < 2048
    ssh_port            = '22'
  ...
  when /sles/
    ...
    if values['service'].to_s.match(/sles_12_[1-9]|sles_15/)
      boot_command = if values['vmnetwork'].to_s.match(/hostonly|bridged/)
                       '<esc><enter><wait> linux<wait>' \
                       ...
                       '<esc><enter><wait> linux text install=cd:/ textmode=1 insecure=1' \
  ...
  when /purity/
    ...
    wait_time1 = if (values['host-os-cpu'].to_i > 6) && (values['host-os-memory'].to_i > 16)
                   '<wait360>'
                 else
                   '<wait500>'
                 end
    ...
    if !File.exist?(values['setup'])
      ...
      user = `cat '#{script_file}' |grep Username |awk '{print $3}'`.chomp
      pass = `cat '#{script_file}' |grep Password |awk '{print $3}'`.chomp
    end
    other_ips = ''
    other_net = ''
    values['ip'] = values['ip'].split(/,/)[0] if values['ip'].to_s.match(/,/)
    if values['answers']['eth1_ip']&.value&.match(/[0-9]/)
      ...
      other_net += "<wait3>echo 'auto #{ethernet}' >> #{net_config}<enter>" \
  ...
  when /debian|ubuntu/
    ...
    if values['livecd'] == true
      ...
      if values['release'].to_i >= 20
        ...
        boot_header = if !values['service'].to_s.match(/22_04_3/)
                        '<wait>e<wait><down><wait><down><wait><down><wait><leftCtrlOn>e<leftCtrlOff>' \
      ...
      boot_header += 'net.ifnames=0 biosdevname=0 ' if values['biosdevnames'] == true
      boot_command = if values['release'].to_i >= 20
                       "#{boot_header}autoinstall ds='nocloud-net;s=http://#{ks_ip}:#{values['httpport']}/' #{boot_footer}"
                     else
                       boot_header +
      ...
      kernel_string = if values['biosdevnames'] == true
      ...
      if values['vmnetwork'].to_s.match(/hostonly|bridged/)
        boot_command = "#{boot_header}<wait>/install/vmlinuz<wait> debian-installer/language=#{values['answers']['language'].value} debian-installer/country=#{values['answers']['country'].value} keyboard-configuration/layoutcode=#{values['answers']['layout'].value} <wait>interface=#{values['answers']['nic'].value} netcfg/disable_autoconfig=#{values['answers']['disable_autoconfig'].value} netcfg/disable_dhcp=#{values['answers']['disable_dhcp'].value} hostname=#{values['name']} <wait>netcfg/get_ipaddress=#{values['ip']} netcfg/get_netmask=#{values['answers']['netmask'].value} netcfg/get_gateway=#{values['answers']['gateway'].value} netcfg/get_nameservers=#{values['answers']['nameserver'].value} netcfg/get_domain=#{values['answers']['domain'].value} <wait>preseed/url=#{ks_url} initrd=/install/initrd.gz #{kernel_string}-- #{boot_footer}"
      else
  ...
  when /vsphere|esx|vmware/
    boot_wait = '2s'
    virtual_dev = 'pvscsi' if values['vm'].to_s.match(/fusion/)
    hwvirtex = 'on'
    ks_file  = "#{values['vm']}/#{values['name']}/#{values['name']}.cfg"
    ks_url   = "http://#{ks_ip}:#{values['httpport']}/" + ks_file
    if values['vm'].to_s.match(/fusion/)
  ...
  when /fedora|[el,centos,rocky,alma]_[8,9]/
    ...
    boot_command = if values['vmnetwork'].to_s.match(/hostonly|bridged/)
                     "<tab><wait><bs><bs><bs><bs><bs><bs>=0 #{kernel_string}inst.text inst.method=cdrom inst.repo=cdrom:/dev/sr0 inst.ks=#{ks_url} ip=#{values['ip']}::#{values['vmgateway']}:#{values['netmask']}:#{values['name']}:eth0:off<enter><wait>"
                   else
                     "<tab><wait><bs><bs><bs><bs><bs><bs>=0 #{kernel_string}inst.text inst.method=cdrom inst.repo=cdrom:/dev/sr0 inst.ks=#{ks_url} ip=dhcp<enter><wait>"
                   end
  ...
  #    boot_command  = "<esc><wait> linux text install ks="+ks_url+" ksdevice=eno16777736 "+"ip="+values['ip']+" netmask="+values['netmask']+" gateway="+values['vmgateway']+"<enter><wait>"
    ...
    ks_url  = "http://#{ks_ip}:#{values['httpport']}/" + ks_file
    boot_command = if values['vmnetwork'].to_s.match(/hostonly|bridged/)
                     "<esc><wait> linux #{kernel_string}text install ks=#{ks_url} ip=#{values['ip']} netmask=#{values['netmask']} gateway=#{values['vmgateway']}<enter><wait>"
                   else
                     "<esc><wait> linux #{kernel_string}text install ks=#{ks_url}<enter><wait>"
                   end
    values['guest'] = values['guest'].join if values['guest'].instance_of?(Array)
  ...
  if values['checksum'] == true
    ...
    install_md5 = if File.exist?(md5_file)
                    File.readlines(md5_file)[0]
                  else
                    `md5 "#{values['file']}" |awk '{print $4}'`.chomp
                  end
  ...
  install_checksum = if install_checksum_type == 'none'
                       'none'
                     else
                       "#{install_checksum_type}:#{install_checksum}"
                     end
  headless_mode = 'true' if values['vm'].to_s.match(/kvm/) && values['console'].to_s.match(/text/)
  get_vm_if_name(values)
  if values['service'].to_s.match(/windows/) && values['vm'].to_s.match(/vbox/) && values['vmnetwork'].to_s.match(/hostonly|bridged/)
    warning_message(values, 'Packer with Windows and VirtualBox only works on a NAT network (Packer issue)')
    information_message(values, 'Use the --network=nat option')
    quit(values)
  end
  if !values['guest'] || (values['guest'] = values['empty'])
    values['guest'] = get_vbox_guest_os(values) if values['vm'].to_s.match(/vbox/)
    values['guest'] = get_fusion_guest_os(values) if values['vm'].to_s.match(/fusion/)
  end
  case values['service']
  when /vmware|vsphere|esxi/
    case values['vm']
    when /vbox/
      json_data = if values['vmnetwork'].to_s.match(/hostonly|bridged/)
                    {
                      variables: {
  ...
  when /win/
    ...
    when /parallels/
      if values['vmnetwork'].to_s.match(/hostonly|bridged/)
        json_data = if values['headless'] == true
                      {
                        variables: {
                          hostname: values['name'],
                          net_config: net_config
  ...
  FileUtils.mkdir_p(json_dir) unless Dir.exist?(json_dir)
