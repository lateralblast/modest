...
def populate_cc_user_data(values)
  ...
  values['hostip']
  values['name']
  install_layout = if values['service'].to_s.match(/ubuntu_2[2-4]/)
                     values['answers']['locale'].value.split('.')[0]
                   else
  ...
  install_disk.split(%r{/})[-1] if install_disk.match(%r{/})
  ...
  install_codename = get_code_name_from_release_version(values['release']) unless install_codename.to_s.match(/[a-z]/)
  if values['vmnetwork'].to_s.match(/hostonly/)
    values['hostonlyip']
  elsif disable_dhcp.match(/false/)
    values['hostonlyip']
  else
    values['hostip']
  end
  install_dhcp = if disable_dhcp.match(/false/)
                   'yes'
                 else
                   'no'
                 end
  ...
  if !values['vm'].to_s.match(/mp|multipass/)
    ...
    user_data.push('  updates: security') if install_security.to_s.match(/true/)
    user_data.push('  user-data:')
    user_data.push('    disable-root: false')
    early_exec_data.push("ip addr add #{install_ip}/#{install_cidr} dev #{install_nic}") if disable_dhcp.match(/true/)
  ...
  if (values['copykeys'] == true) && File.exist?(ssh_keyfile) && !values['vm'].to_s.match(/mp|multipass/)
  ...
  unless values['vm'].to_s.match(/mp|multipass/)
    ...
    unless values['vm'].to_s.match(/mp|multipass/)
      ...
      late_exec_data.push("echo 'Port #{install_ssh_port}' >> #{install_target}/etc/ssh/sshd_config") if values['type'].to_s.match(/packer/) && (install_ssh_port.to_i != 22)
      late_exec_data.push("#{in_target}update-grub") if values['serial'] == true || values['biosdevnames'] == true
      late_exec_data.push("#{in_target}reboot") if values['reboot'] == true
    end
  end
...
def output_cc_user_data(values, user_data, early_exec_data, late_exec_data, output_file)
  ...
  if cc_file.match(/[a-z]/) && File.exist?(cc_file)
