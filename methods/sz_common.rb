...
def check_zone_is_installed()
  if options['host-os-uname'].to_s.match(/SunOS/)
    if options['host-os-unamer'].split(/\./)[0].to_i > 10
      exists =  true
    else
      exists = false
    end
...
def list_zone_services(options)
  if options['host-os-uname'].to_s.match(/SunOS/)
    options['version'] = options['host-os-unamer'].split(/\./)[1]
...
def list_zone_isos(options)
  ...
  if iso_list.length > 0
    ...
    if options['host-os-unamep'].to_s.match(/sparc/)
      search_arch = options['host-os-unamep']
    else
      search_arch = "x86"
    end
...
def create_zone_config(options)
  ...
  if not zone_status.match(/#{options['name']}/)
    if options['host-os-unamep'].to_s.match(/i386/)
...
def create_zone(options)
  ...
  if options['host-os-unamer'].match(/11/) and options['release'].to_s.match(/10/)
    if options['host-os-unamep'].to_s.match(/i386/)
    ...
    if not options['image'].to_s.match(/[a-z,A-Z]/)
      ...
      if options['host-os-unamer'].match(/11/) and virtual == true
