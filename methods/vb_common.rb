...
def import_vbox_ova(options)
  ...
  if options['vmnet'].to_s.match(/bridged/)
    vbox_nic = get_bridged_vbox_nic()
    add_bridged_network_to_vbox_vm(options['name'],vbox_nic)
  else
    if_name  = get_bridged_vbox_nic()
    vbox_nic = check_vbox_hostonly_network(if_name,options)
    add_nonbridged_network_to_vbox_vm(options['name'],vbox_nic)
  end
