...
def print_arch_types(values)
  ...
  if values['vm'] =~ /ldom|zone/
    verbose_message(values, "sparc  - 64 bit SPARC")
  end
  verbose_message(values, "")
...
def print_os_types(values)
  ...
  verbose_message(values, "")
  verbose_message(values, "solaris       - Solaris (Sets install type to Jumpstart on Solaris 10, and AI on Solaris 11)")
...
def print_examples(values)
  ...
  if examples.match(/zone|all/)
    ...
    verbose_message(values, "#{values['script']} -Z -c sol10u11z01 -i 192.168.1.171 -f /export/isos/solaris-10u11-x86.bin")
    verbose_message(values, "Configure Branded Zone:")
  ...
  if examples.match(/client|xb|ob|nb|all/)
    ...
    verbose_message(values, "#{values['script']} -B -C -c openbsd55vm01 -e 00:50:56:26:92:d8 -a x86_64 -i 192.168.1.193 -n openbsd_5_5_x86_64")
    verbose_message(values, "Create FreeBSD client:")
    verbose_message(values, "#{values['script']} -B -C -c freebsd10vm01 -e 00:50:56:26:92:d7 -a x86_64 -i 192.168.1.194 -n netbsd_10_0_x86_64")
