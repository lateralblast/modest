...
def create_aws_s3_bucket(values)
  ...
  if s3.bucket(values['bucket']).exists?
    handle_output(values, "Information:\tBucket: #{values['bucket']} already exists")
    s3 = initiate_aws_s3_client(values['access'], values['secret'], values['region'])
    begin
      s3.head_bucket({ bucket: values['bucket'], })
    rescue
      handle_output(values, "Warning:\tDo not have permissions to access bucket: #{values['bucket']}")
      quit(values)
    end
  else
    handle_output(values, "Information:\tCreating S3 bucket: #{values['bucket']}")
...
def show_aws_s3_bucket_acl(values)
  ...
  handle_output(values, "#{values['bucket']}\towner=#{owner}")
  acl.grants.each_with_index do |grantee, counter|
    ...
    handle_output(values, "grants[#{counter}]\towner=#{owner}\temail=#{email}\ttype=#{type}\turi=#{uri}\tid=#{id}\tperms=#{perms}")
  end
  return
end
...
def upload_file_to_aws_bucket(values)
  ...
  if not values['bucket'].to_s.match(/[A-Z]|[a-z]|[0-9]/)
    handle_output(values, "Warning:\tNo Bucket name given")
    values['bucket'] =  values['bucket']
    handle_output(values, "Information:\tSetting Bucket to default bucket '#{values['bucket']}'")
  end
  ...
  handle_output(values, "Information:\tUploading: File '#{values['file']}' with key: '#{values['key']}' to bucket: '#{values['bucket']}'")
...
def download_file_from_aws_bucket(values)
  ...
  s3 = initiate_aws_s3_client(values)
  handle_output(values, "Information:\tDownloading: Key '#{values['key']}' from bucket: '#{values['bucket']}' to file: '#{values['file']}'")
