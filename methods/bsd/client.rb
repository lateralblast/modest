...
def configure_xb_pxe_client(values)
  ...
  if values['service'].to_s.match(/openbsd/)
    ...
    pxeboot_file  = values['service'] + '/' + values['version'] + '/' + values['arch'].gsub(/x86_64/,
                                                                                            'amd64') + '/pxeboot'
  else
  ...
  if values['service'].to_s.match(/coreos/)
    ...
    unless File.exist?(ldlinux_link)
...
def output_coreos_client_profile(values)
  ...
  values['answers']['admin_group'].value
  admin_user    = values['answers']['admin_user'].value
  admin_crypt   = values['answers']['admin_crypt'].value
  values['answers']['admin_home'].value
  values['answers']['admin_uid'].value
  values['answers']['admin_gid'].value
