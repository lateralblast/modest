...
def check_default_route(values)
  ...
  if !output.match(/[0-9]/)
    handle_output(values, "Warning:\tNo default route exists")
    if values['test'] != true
      quit(values)
    end
  end
...
def check_dhcpd4_conf(values)
  if values['host-os-uname'].to_s.match(/SunOS/)
    ...
    if values['verbose'] == true
      handle_output(values, "Checking:\t#{file} exists")
    end
...
def check_ai_base_dir(values)
  if values['verbose'] == true
    handle_output(values, "Checking:\t#{values['aibasedir']}")
  end
...
def check_version_dir(values, dir_name, repo_version)
  ...
  handle_output(values, "Checking:\t#{full_version_dir}")
...
def configure_ai_services(values, iso_repo_version)
  handle_output(values, "Information:\tCreating AI services")
  arch_list= []
  ...
  if output.to_s.match(/maintenance/)
    handle_output(values, "Warning:\tDHCP Server is in maintenance mode")
    quit(values)
  end
  arch_list.each do |sys_arch|
    ...
    if values['host-os-uname'].to_s.match(/SunOS/)
      ...
      if not File.exist?(ai_file)
        handle_output(values, "Warning:\tAI ISO file #{ai_file} not found for architecture #{lc_arch}")
      else
        if values['host-os-uname'].to_s.match(/Darwin/)
          ...
          if output.match(/Resource busy/)
            handle_output(values, "Warning:\t ISO already mounted")
            quit(values)
          end
        end
      end
...
def get_ai_solaris_release(values)
  ...
  if File.directory?(manifest_dir)
    ...
    if File.exist?(release_file)
      ...
      handle_output(values, "Warning:\tCould not find #{release_file}")
      handle_output(values, "Warning:\tCould not verify solaris release from repository")
      handle_output(values, "Information:\tSetting Solaris release to 11")
      iso_repo_version="11"
    end
  end
...
def configure_ai_server(values)
  ...
  if values['service'].to_s.match(/[a-z,A-Z]/)
    if services_list.match(/#{values['service']}/)
      handle_output(values, "Warning:\tService #{values['service']} already exists")
      quit(values)
    end
  end
  ...
  if not iso_list[0]
    handle_output(values, "Warning:\tNo suitable ISOs found")
    quit(values)
  end
  iso_list.each do |file_name|
    if File.exist?(file_name)
      if !file_name.match(/repo/)
        handle_output(values, "Warning:\tISO #{file_name} does not appear to be a valid Solaris distribution")
        quit(values)
      end
    else
      handle_output(values, "Warning:\tISO #{file_name}'' does} not exist")
      quit(values)
    end
