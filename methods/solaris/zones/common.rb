...
def check_zone_is_installed(values)
  if values['host-os-uname'].to_s.match(/SunOS/)
    values['host-os-unamer'].split(/\./)[0].to_i > 10
  else
    false
  end
end
...
def get_zone_image_info(_option)
  ...
  image_arch = 'i386' if image_arch.match(/x86/)
  values['service'] = "#{image_os}_#{image_version.gsub(/\./, '_')}_#{image_arch}"
  [image_version, image_arch, values['service']]
end
...
def list_zone_isos(values)
  ...
  if iso_list.length.positive?
    ...
    search_arch = if values['host-os-unamep'].to_s.match(/sparc/)
                    values['host-os-unamep']
                  else
                    'x86'
                  end
    iso_list.each do |image_file|
      image_file = image_file.chomp
      next unless image_filematch(/^solaris/) && image_file.match(/bin$/)

      next unless image_file.match(/#{search_arch}/)
...
def standard_zone_post_install(values)
  ...
  if File.directory?(values['zonedir'])
    ...
    info.each do |line|
      field = line.split(':')
      file.write(line) if (field[0] != 'root') && (field[0] != admin_username.to_s)
      next unless field[0].to_s.match(/root/)
    ...
    system("rm #{key_file}") if File.exist?(key_file)
    [rsa_file, dsa_file].each do |pub_file|
      next unless File.exist?(pub_file)
...
def create_branded_zone(_option)
...
def create_zone(values)
  ...
  if values['service'].to_s.match(/[a-z,A-Z]/)
    ...
    image_arch = 'x86' if image_arch.match(/i386/)
    values['image'] = "solaris-#{image_version}-#{image_arch}.bin"
  end
  if values['host-os-unamer'].match(/11/) && values['release'].to_s.match(/10/)
    branded_file = if values['host-os-unamep'].to_s.match(/i386/)
                     "#{branded_dir}solaris-10u11-x86.bin"
                   else
                     "#{branded_dir}solaris-10u11-sparc.bin"
                   end
    check_fs_exists(values, branded_dir)
    print_branded_zone_info(values) unless File.exists(branded_file)
    create_branded_zone(values['image'], values['ip'], values['vmnic'], values['name'], values['release'])
  elsif !values['image'].to_s.match(/[a-z,A-Z]/)
    values['zone'] = create_zone_config(values['name'], values['ip'])
    install_zone(values['name'], values['zone'])
    standard_zone_post_install(values['name'], values['release'])
  else
    print_branded_zone_info(values) unless File.exist?(values['image'])
    create_zone_config(values['name'], values['ip'])
    if values['host-os-unamer'].match(/11/) && (virtual == true)
  ...
  boot_zone(values) if values['serial'] == true
...
def boot_zone(values)
  ...
  execute_command(values, message, command)
  system("zlogin #{values['name']}") if values['serial'] == true
  nil
end
...
def configure_zone(values)
  check_same_arch(values['arch']) if values['arch'].to_s.match(/[a-z,A-Z]/)
  values['release'] = values['host-os-unamer'] if !values['image'].to_s.match(/[a-z,A-Z]/) && !(values['service'].to_s.match(/[a-z,A-Z]/) && !values['release'].to_s.match(/[0-9]/))
  ...
  unless File.directory?(values['zonedir'])
