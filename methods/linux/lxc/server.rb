  ...
  if values['host-os-uname'].to_s.match(/Linux/) && Dir.exist?(values['lxcimagedir'])
    ...
    if image_list.length.positive?
      ...
      image_list.each do |image_name|
        next unless image_name.match(/tar/)
        ...
        values['service'] = if image_info[3]
                              "#{image_os.gsub(/ /, '')}_#{image_ver.gsub(/\./, '_')}_#{image_arch}_#{image_info[3]}"
                            else
                              "#{image_os.gsub(/ /, '')}_#{image_ver.gsub(/\./, '_')}_#{image_arch}"
                            end
        if values['output'].to_s.match(/html/)
...
def configure_ubuntu_lxc_server(values, server_type)
  ...
  if server_type.match(/public/)
    ...
    unless output.match(/bridge_ports/)
...
def configure_lxc_server(values, server_type)
  ...
  configure_ubuntu_lxc_server(values, server_type) if values['host-os-unamea'].match(/Ubuntu/)
