...
def create_ubuntu_lxc_config(values)
  ...
  info.each do |line|
    field = line.split(':')
    file.write(line) if (field[0] != 'ubuntu') && (field[0] != user_username.to_s)
  end
  ...
  info.each do |line|
    field = line.split(':')
    file.write(line) if (field[0] != 'ubuntu') && (field[0] != 'root') && (field[0] != user_username.to_s)
    next unless field[0] == 'root'
...
def create_standard_lxc(values)
  ...
  command = "lxc-create -t ubuntu -n #{values['name']}" if values['host-os-unamea'].match(/Ubuntu/)
  execute_command(values, message, command)
  nil
end
...
def configure_lxc(values)
  ...
  if !values['service'].to_s.match(/[a-z,A-Z]/) && !values['image'].to_s.match(/[a-z,A-Z]/)
    ...
    create_ubuntu_lxc_config(values) if values['host-os-unamea'].match(/Ubuntu/)
    create_centos_lxc_config(values) if values['host-os-unamea'].match(/RedHat|Centos/)
  else
    if values['service'].to_s.match(/[a-z,A-Z]/)
      values['image'] =
        $lxc_image_dir + '/' + values['service'].gsub(/([0-9])_([0-9])/, '\1.\2').gsub(/_/, '-').gsub(/x86.64/,
                                                                                                      'x86_64') + '.tar.gz'
    end
    if values['image'].to_s.match(/[a-z,A-Z]/) && !File.exist?(values['image'])
